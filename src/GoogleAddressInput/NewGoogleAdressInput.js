...
import styles from './NewGoogleAddressInput.scss';

export const NewGoogleAddressInputHandler = {
  geocode: 'geocode',
  places: 'places',
};
 ...
 */
class NewGoogleAddressInput extends React.Component {
  constructor(props) {
    super(props);
        ...
        <InputWithOptions
          ...
          fixedFooter={
            suggestions.length && this.props.poweredByGoogle
              ? NewGoogleAddressInput.getGoogleFooter()
              : null
          }
          suffix={suffix}
  ...
  onSet(value) {
    ...
    if (
      handler === NewGoogleAddressInputHandler.places &&
      suggestion &&
      suggestion.place_id
    ) {
      const request = {
    ...
    handlerCall
      .then(results => {
        ...
        if (results.length === 0) {
          console.error(
            `[NewGoogleAddressInput] handler (${handler}) returned no results on`,
            value,
          );
          this.props.onSet && this.props.onSet(null);
      ...
      .catch(e => {
        console.error(
          `[NewGoogleAddressInput] handler (${handler}) failed on`,
          value,
          e.message,
        );
        this.props.onSet && this.props.onSet(null);
...
NewGoogleAddressInput.displayName = 'NewGoogleAddressInput';

NewGoogleAddressInput.defaultProps = {
  ...
  handler: NewGoogleAddressInputHandler.geocode,
};

NewGoogleAddressInput.propTypes = {
  ...
  handler: PropTypes.oneOf([
    NewGoogleAddressInputHandler.geocode,
    NewGoogleAddressInputHandler.places,
  ]),
};

export default NewGoogleAddressInput;
